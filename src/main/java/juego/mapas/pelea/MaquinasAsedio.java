package main.java.juego.mapas.pelea;

import javafx.scene.image.Image;
import main.java.Utils.Posicion;


public class MaquinasAsedio extends Posicion {
    private static int lastId=1;
    private int id;
    private MaquinasAsedioPreCargadas soldadosPreCargados;
    private int cantidad;
    int municion;

    public MaquinasAsedio(MaquinasAsedioPreCargadas soldadosPreCargados, int cantidad, int municion, int filas, int columnas) {
        super(filas, columnas);
        this.id = lastId++;
        this.soldadosPreCargados = soldadosPreCargados;
        this.cantidad = cantidad;
        this.municion = municion;
    }

    public int getId() {
        return id;
    }

    public int getCantidad() {
        return cantidad;
    }

    public MaquinasAsedioPreCargadas getsoldadosPreCargados() {
        return soldadosPreCargados;
    }

    public void setCantidad(int cantidad) {
        this.cantidad = cantidad;
    }

    public Image getImage() {
        return soldadosPreCargados.getImage();
    }

    public Image getImageClicable() {
        return soldadosPreCargados.getImageClicable();
    }

    @Override
    public String toString() {
        return id + "_ " + soldadosPreCargados.getNombre() + "=" + cantidad;
    }
}
