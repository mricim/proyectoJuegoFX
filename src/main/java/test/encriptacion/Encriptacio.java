...
public class Encriptacio {
    ...
    public static String encrypt(String key, String iv, String data) {
        try {
            if (key.length() < Encriptacio.CIPHER_KEY_LEN) {
                int numPad = Encriptacio.CIPHER_KEY_LEN - key.length();

                for(int i = 0; i < numPad; i++){
                    key += "0"; //0 pad to len 16 bytes
                }

            } else if (key.length() > Encriptacio.CIPHER_KEY_LEN) {
    ...
    public static void main(String[] args) {
        ...
        String initVector = "aaaaaaaaaaaaaaaa"; // 16 bytes IV

        System.out.println(encrypt(key, initVector, "pepe"));
        System.out.println(decrypt(key, encrypt(key,initVector, "pepe")));
    }
}